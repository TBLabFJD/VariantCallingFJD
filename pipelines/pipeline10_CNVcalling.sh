...
if echo "$methods" | grep -q "QC"; then
	...
	if [ "$n" != "1" ] || [ "$status" != "PASSED" ]; then 
		...
		s1="$?"
		for file in $samples; do cp ${file}*bai $bamF_pass/.; done
		s2="$?"
...
if echo "$methods" | grep -q "ED"; then
	start=`date +%s`
	printf "\nRunning ExomeDepth"
	Rscript $tasksPath/exomeDepth.R -d $bamF_pass -o $MDAP -b $finalpanel -n $run -s $SAMPLEFILE
...
if echo "$methods" | grep -q "MA"; then
	...
	printf "\nMerging CNV calling results from alternative methods\n"
	Rscript $tasksPath/CNV_result_mixer.R -o $MDAP -n $run -b $finalpanel -s $SAMPLEFILE -d $bamF 
