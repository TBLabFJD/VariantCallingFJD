...
gatk VariantFiltration --tmp-dir /tmp/tmp \
  ...
  -O /batchx/output/${outputPrefix}.filtered.snp.vcf
...
gatk SelectVariants --tmp-dir /tmp/tmp \
  ...
  -O /batchx/output/${outputPrefix}.indel.vcf

rm -rf /tmp/tmp/*

#4.Apply the filters to the INDEL's callset.
gatk VariantFiltration --tmp-dir /tmp/tmp \
  -V /batchx/output/${outputPrefix}.indel.vcf \
  -filter "QD < 2.0" --filter-name "QD2" \
  -filter "QUAL < 30.0" --filter-name "QUAL30" \
  -filter "FS > 200.0" --filter-name "FS200" \
  -filter "ReadPosRankSum < -20.0" --filter-name "ReadPosRankSum-20" \
  -O /batchx/output/${outputPrefix}.filtered.indel.vcf

rm -rf /tmp/tmp/*
# Combine Variants after using SNPS and INDELS filtering into a single file and get it ready for annotation.

vcf=/batchx/output/${outputPrefix}.gatkLabeled.vcf
gatk MergeVcfs --TMP_DIR /tmp/tmp \
  -R $refFile \
  -I /batchx/output/${outputPrefix}.filtered.snp.vcf \
  -I /batchx/output/${outputPrefix}.filtered.indel.vcf \
  -O $vcf
