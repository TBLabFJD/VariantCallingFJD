...
output_dir <- paste(opt$outputdir,"/copy_number_variation_data/",sep="")
bedFile <- opt$bed
...
#************************#
exomedepth_output <- paste(output_dir, "/ExomeDepth/", 'exomedepth', '_results.txt', sep = '')
tryCatch(
  {
...
#***********************#
convading_output <- paste(output_dir, "/CoNVaDING/", 'CoNVaDING', '_results.txt', sep = '')
tryCatch(
  {
...
# Import Codex2 data #
#********************#
codex2_output <- paste(output_dir, "/CODEX2/", 'CODEX2', '_results.txt', sep = '')
tryCatch(
  {
...
total_samples <- unique(gsub("_.*$", "", list.files(opt$dir)))
if(opt$samples!="all"){
...
file = paste(projectname, "_combined.txt", sep = ""), append = TRUE)
...
bedout <- bedFile_data
bedout$AVG_POS <- NULL
write.table(bedout, file = paste(projectname, "_extended.bed", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE)
