...
# mitochondrial chromosomes are removed
# gene annotation is not considered
...
colnamesbed = c("chromosome", "start",  "end")
...
if(opt$bed=="genome"){
  ...
  if(length(colnames(myexons))<3){
    cat("ERROR: bed file lacking columns")
    quit(status = 1)}
  if(length(colnames(myexons))==3){
    myexons$names = as.factor(1:nrow(myexons)) 
  }
...
reference.fasta <- '/home/proyectos/bioinfo/references/hg19/ucsc.hg19.fasta'
...
#load(file = paste('exomedepth',"_counts.RData", sep=''))
#load("/home/proyectos/bioinfo/lodela/SVbenchmark/CODEX2/individual_copynumber/copy_number_variation_data/ExomeDepth/exomedepth_counts.RData")
...
colnames(ExomeCount.mat) = sapply(colnames(ExomeCount.mat), function(x) sub("_alignment.bam", "",strsplit(x, split = "_",  fixed = T)[[1]][1]))
  ...
  if(samplename %in% samples){
    ...
    if (correlation > 0.97){
      ...
      if (nrow(all.exons@CNV.calls) > 0) {
        ...
        CNVcalls_sorted_total = rbind( CNVcalls_sorted_total, data.frame(samplename=rep(samplename, nrow(CNVcalls_sorted)),CNVcalls_sorted))
...
save(CNVcalls_sorted_total, file = paste('ExomeDepth',"_CNV.RData", sep=''))
