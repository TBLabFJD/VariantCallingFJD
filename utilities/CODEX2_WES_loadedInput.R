...
print(cnv_index1)
save(normObj.null, file = paste('CODEX2',"_norm.RData", sep=''))
...
for (chr in unique(seqnames(ref_qc))){
  ...
  Yhat.chr <- lapply(Yhat, function(x) x[chr.index,])  

  finalcall.CBS[[chr]] <- segmentCBS(Y_qc = Y_qc[chr.index,],  # recommended
                              Yhat = Yhat.chr, optK = which.max(BIC),
                              K = 1:5,
...
save(finalcall.CBS, file = paste('CODEX2',"_CNV_nr.RData", sep=''))
