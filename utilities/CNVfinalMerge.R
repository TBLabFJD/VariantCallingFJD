...
cnv_dir=paste(opt$resultsdir, "/copy_number_variation_data/", sep='')
mixer=paste(cnv_dir, opt$run, "_combined.txt", sep='')
annotsv=paste(cnv_dir, opt$run, "_combinedAnnotated.tsv", sep='')
...
final_output= paste(cnv_dir,  opt$run, "_final.txt", sep="")
