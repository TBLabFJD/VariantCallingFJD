...
if echo "$methods" | grep -q "MA"; then
	...
	printf '\n\nAnnotSV CNV annotation... '



	annotsv -SVinputFile  $CNV/${run}_combined.vcf -outputfile $CNV/${run}_combinedAnnotated.txt



	if [ "$?" = "0" ]; then
		printf '\nEXIT STATUS: 0'
		printf "\nSV ANNOTATION FOR FILE $CNV/${run}"_combinedAnnotated.txt" DONE"
	...
	echo -e "\nMerging mixer and AnnotSV output files..."


	Rscript CNVfinalMerge.R -d $MDAP -r $run
