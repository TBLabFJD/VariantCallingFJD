...
def main():
			...
			if args.analysis in ["cnv","all"] or sample_name in sample_namesT: # just analyse if contained in samples_files or we are running CNVs	
				...
				if sampleAnalysis!=None:
					...
					if args.local:	# LOCAL
						...
						subprocess.call(myargs_pipe1, stdout=stdout_f, stderr=stderr_f) # mapping 
						subprocess.call(myargs_pipe2, stdout=stdout_f, stderr=stderr_f) # bam processing + snp calling + snp filtering
	...
	if args.cvcf:
		...
		myargs_desc = ["SCRIPT", "OUTPUT DIR", "RUN LABEL", "NUMBER THREADS", "GENOME BUNDLE","PEDIGREE","LOCAL","PATHOLOGY", "KEEP SINGLE VCFs", "GENE LIST", "SOFTPATH"]
		myargs_cvcf = [utilitiesPath+"combinedGenotyping.sh", args.output, run, str(args.threads), args.genome, args.pedigree, str(args.local), str(args.pathology), str(args.single), str(args.genefilter), softwarePath]
