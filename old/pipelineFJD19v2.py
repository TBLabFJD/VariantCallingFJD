...
def sbatch(job_name, folder_out, command, mem=4, time=400, threads=5, mail=None, dep='', wait = '', queue=None, typedep=None):
...
def main():
	...
	if args.skipMapping == False or args.analysis in ["snp","all"]: # mapping or snps
		...
		for sample_name in sample_names: # four options: (1) no sample analysis; (2) just mapping; (3) just snp; (4) mapping + snp
			...
			if args.analysis in ["cnv","all"] or sample_name in sample_namesT: # sample is analyzed if contained in samples_files or we are running CNVs (need of controls). Depending on that we assign the type of analysis to the sample
				...
				### SNP GENOTYPING (WITH OR WITHOUT PROCESSING)
				...
				if sampleAnalysis!=None:
					...
					# print arguments
					myargs_desc = ["INPUT FOLDER", "OUTPUT DIR", "SAMPLE LABEL", "N THREADS", "RUN LABEL", "PANEL BED FILE", "BASESPACE DOWNLOAD", "CONCATENATION", "INPUT DIRECTORY", "ANALYSIS TYPE", "COMBINED VCF", "SKIPPING MAPPING STEP", "GENOME BUNDLE", "LOCAL", "PATHOLOGY", "INTERVALS", "REMOVE DUPLICATES", "REMOVE BAM FILES", "GENE LIST", "PADDING", "BASESPACE USER", "SOFTPATH"]
					...
					myargs_pipe5 = [pipelinesPath+"pipeline5_LohAnnotationOutput.sh", args.output, sample_name, str(args.threads), run, args.panel, str(args.cvcf), args.genome, str(args.local), args.pathology, args.genefilter, softwarePath]
					...
					if sampleAnalysis=="mapping": # SBATCH AND SAVE JOB IDS FOR MAPPING SAMPLES
						...
						jobname_pipe2 =  "processing_"+sample_name
					...
					elif sampleAnalysis=="mapping_snp": # SBATCH AND SAVE JOB IDS FOR MAPPING AND SNP CALLING SAMPLES
						jobname_pipe1 =  "mapping"+"_"+sample_name
						...
						jobname_pipe2 =  "processing_snp_"+sample_name
						jobid=sbatch(jobname_pipe2, args.output, ' '.join(myargs_pipe2), time=args.time, mem=args.memory, threads=2, mail=args.mail, dep=tmpjobid) # bam processing + snp calling + snp filtering
						jobid_list_snp.append(jobid)
						sys.stdout.write("JOB ID PROCESSING AND SNV CALLING: %s\n" %(jobid))


						if str(args.cvcf)=="False":

							jobname_pipe5 =  "annot_"+sample_name
							jobid=sbatch(jobname_pipe5, args.output, ' '.join(myargs_pipe5), time=args.time, mem=args.memory, threads=args.threads, mail=args.mail, dep=jobid) # vcf annotation and complementary analysis to generate full output
							sys.stdout.write("JOB ID SNV ANNOTATION: %s\n" %(jobid))


					elif sampleAnalysis=="snp": # SBATCH AND SAVE JOB IDS FOR SNP

						jobname_pipe2 =  "snp_"+sample_name
	...
	if args.cvcf:
		...
		myargs = [args.output, run,  args.genome]
		myargs_pipe = [tasksPath+"combinedGVCF.sh"]+myargs
		depJobs = ':'.join(jobid_list_snp)  	

		job_name="combinedGVCF_"+run
		#jobid=sbatch(job_name, args.output, ' '.join(myargs_pipe), time=args.time, mem=args.memory, threads=2, mail=args.mail, dep=depJobs) 
		sys.stdout.write("JOB ID combinedGVCF: %s\n" %(jobid))

		

		# Genotyping g.vcf

		myargs = [str(args.local), run, args.output, run, args.genome]
		myargs_pipe = [tasksPath+"genotyping.sh"]+myargs

		job_name="genotypeGVCF_"+run
		...
		myargs = [str(args.local), run, args.output, run, args.genome, "HF", str(args.cvcf)]
		myargs_pipe = [tasksPath+"SNVfiltering.sh"]+myargs

		job_name="filteringHF_"+run
		#jobid=sbatch(job_name, args.output, ' '.join(myargs_pipe), time=args.time, mem=args.memory, threads=2, mail=args.mail, dep=jobid) 
		sys.stdout.write("JOB ID genotyping: %s\n" %(jobid))
		...
		myargs = [str(args.local), run, args.output, args.pedigree, args.genome]
		myargs_pipe = [tasksPath+"genotypeRefinement.sh"]+myargs

		job_name="genotypeRefinement_"+run
		#jobid=sbatch(job_name, args.output, ' '.join(myargs_pipe), time=args.time, mem=args.memory, threads=2, mail=args.mail, dep=jobid) 
		sys.stdout.write("JOB ID genotype refinement: %s\n" %(jobid))
		...
		myargs =  [args.output, run, str(args.threads), run, args.panel, str(args.cvcf), args.genome, str(args.local), args.pathology, args.genefilter, softwarePath]
		myargs_pipe = [pipelinesPath+"pipeline5_LohAnnotationOutput.sh"]+myargs

		job_name="annotation_"+run
		jobid=sbatch(job_name, args.output, ' '.join(myargs_pipe), time=args.time, mem=args.memory, threads=args.threads, mail=args.mail, dep=jobid) 
		sys.stdout.write("JOB ID SNV ANNOTATION: %s\n" %(jobid))
